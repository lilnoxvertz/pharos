...
class Pharos {
    ...
    static async verifyTask(walletAddress, token, proxy, txhash, taskId) {
        ...
        while (!complete && attempt < maxAttempt) {
            try {
                yap.delay(`[PHAROS] ${truncatedAddress} is submitting task id ${taskId}`)
                const response = await fetch(url, {
                    method: "POST",
                    headers: header,
                    body: JSON.stringify(payload),
                ...
                if (!response.ok) {
                    attempt++
                    yap.error(`[PHRAOS] failed getting response`)
                    await delay(3)
                    continue
                }
                ...
                if (code !== 0) {
                    ...
                    yap.error(`[PHAROS] ${truncatedAddress} Failed verifying task id ${taskId}: ${errorMsg}`)
                    await delay(3)
                    continue
                }

                yap.success(`[PHAROS] ${truncatedAddress} Successfully verifying task id ${taskId}`)
                return true
            } catch (error) {
                yap.error(`[Pharos] ${truncatedAddress} Error when trying to verify task id ${taskId}: ${error}`)
            }
        }

        if (!complete && attempt === maxAttempt) {
            return false
        }
    }
